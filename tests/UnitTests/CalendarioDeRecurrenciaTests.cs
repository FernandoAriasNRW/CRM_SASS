...
{
    private static RecurrencePattern Patron(string frecuencia, int intervalo, DateOnly desde)
    ...
    public void La_diaria_suma_dias(int intervalo, string esperada)
    {
        ...
        RecurrenceCalendar.Next(desde, Patron(RecurrencePattern.Frequencies.Daily, intervalo, desde))
    ...
    public void La_semanal_suma_semanas_y_cae_el_mismo_dia(int intervalo, string esperada)
    {
        ...
        var siguiente = RecurrenceCalendar.Next(desde, Patron(RecurrencePattern.Frequencies.Weekly, intervalo, desde));
    ...
    public void La_mensual_conserva_el_dia_del_mes()
    {
        ...
        RecurrenceCalendar.Next(desde, Patron(RecurrencePattern.Frequencies.Monthly, 1, desde))
    ...
    public void La_mensual_del_31_se_recorta_en_febrero_y_vuelve_al_31_en_marzo()
    {
        ...
        var febrero = RecurrenceCalendar.Next(new DateOnly(2026, 1, 31), patron);
        febrero.Should().Be(new DateOnly(2026, 2, 28), "2026 no es bisiesto");

        var marzo = RecurrenceCalendar.Next(febrero, patron);
        marzo.Should().Be(new DateOnly(2026, 3, 31), "la serie recupera su día, no se queda en 28");

        var abril = RecurrenceCalendar.Next(marzo, patron);
    ...
    public void Un_año_entero_de_una_serie_del_31_siempre_cae_a_fin_de_mes()
    {
        ...
        {
            fecha = RecurrenceCalendar.Next(fecha, patron);
    ...
    {
        var crear = () => new RecurrencePattern("Trimestral", 1, DateOnly.FromDateTime(DateTime.UtcNow), null);

        crear.Should().Throw<InvalidOperationException>().WithMessage("*Daily, Weekly o Monthly*");
    }
    ...
    {
        var crear = () => new RecurrencePattern(
            RecurrencePattern.Frequencies.Daily, intervalo, DateOnly.FromDateTime(DateTime.UtcNow), null);
